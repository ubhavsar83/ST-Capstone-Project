...
# Load the dataset
data_path = 's_bucks.csv'
s_bucks = pd.read_csv(data_path)
s_bucks['datetime'] = pd.to_datetime(s_bucks['datetime'])

# Data converting
s_bucks['day_of_week'] = s_bucks['datetime'].dt.dayofweek
s_bucks['month'] = s_bucks['datetime'].dt.month
s_bucks['year'] = s_bucks['datetime'].dt.year

# Handling outliers
Q1, Q3 = s_bucks['close'].quantile([0.25, 0.75])
IQR = Q3 - Q1
lower_bound, upper_bound = Q1 - 1.5 * IQR, Q3 + 1.5 * IQR
filtered_s_bucks = s_bucks[(s_bucks['close'] >= lower_bound) & (s_bucks['close'] <= upper_bound)]

# Data splitting for features and target
X = filtered_s_bucks[['open', 'high', 'low', 'volume', 'day_of_week', 'month', 'year']]
y = filtered_s_bucks['close']
...
def predict():
    ...
    features = [[open_price, high_price, low_price, volume,
                 s_bucks['datetime'].dt.dayofweek.iloc[0],  # Assuming today's date for simplicity
                 s_bucks['datetime'].dt.month.iloc[0],
                 s_bucks['datetime'].dt.year.iloc[0]]]
